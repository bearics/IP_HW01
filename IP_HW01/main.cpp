...
void scaleRAW(unsigned char** data, int sx, int sy, double scale)
{
	...
	for (int y = 0; y < new_sy; y++)
	{
		...
		{
			outData[y][x] = bilinearInterpolation(data, (double)x / scale , (double)y / scale, sx, sy);
		}
	}
...
int main(void)
{
	...
	scaleRAW(data, 256, 256, 436.0 / 256.0);	// scale image
	scaleRAW(data, 256, 256, 512.0 / 256.0);	// scale image
	rotateRAW(data, 256, 256, 30.0);	// rotate image
	rotateRAW(data, 256, 256, 45.0);	// rotate image
	rotateRAW(data, 256, 256, 60.0);	// rotate image
